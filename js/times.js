  ...
  jQuery(document).ready(function($){
    ...
    var disEE=120, //esta variable define la distancia entre estaciones en pixeles
        max=disEE*(data[0].paradas.length)+disEE,
        nEstaciones=data[0].paradas.length;
        console.log(data[0]);
        ...
        $.getJSON("http://localhost:8000/data/busesR1.json").
             ...
             $.bus = dataB;
              var nBuses=dataB.length;
                  it =new itinerario(nBuses,nEstaciones,disEE,data[0].paradas);
  ...
  //  for (var i = 0; i < 5; i++) {
    ...
    a=0;
       ...
       for (var i = 0; i < data[0].paradas.length; i++) {
         var li = document.createElement("li");
         li.innerHTML=data[0].paradas[i].nombre;
         ...
         lineG[0].appendChild(li);
                  ...
                  var estaciones=lineG.find('li')
                   ...
                   for (i = 0; i < estaciones.length; i++) {
                     setPosTimelineE(i,disEE,estaciones.eq(i));
                   }
...
/*
agradecimientos a la pagina https://codyhouse.co/gem/horizontal-timeline/ por el siguiente metodo
*/
